...
function WindowChrome({
  ...
  return (
    <div
      className={cn(
        'overflow-hidden rounded-[10px] border border-[rgba(55,53,47,0.12)] bg-background shadow-[0_10px_30px_-18px_rgba(15,15,15,0.45)] dark:border-[rgba(255,255,255,0.1)]',
        className,
      )}
    >
      <div className="flex items-center gap-2 border-b border-[rgba(55,53,47,0.08)] px-3 py-2 dark:border-[rgba(255,255,255,0.08)]">
        <span className="flex gap-1" aria-hidden>
          <span className="h-2 w-2 rounded-full bg-[#ff5f57]" />
          <span className="h-2 w-2 rounded-full bg-[#febc2e]" />
          <span className="h-2 w-2 rounded-full bg-[#28c840]" />
        </span>
        <span className="min-w-0 flex-1 truncate rounded-[4px] bg-[rgba(55,53,47,0.05)] px-2 py-0.5 text-center text-[10px] text-muted-foreground dark:bg-[rgba(255,255,255,0.05)]">
          {title}
        </span>
        <span className={cn('h-2 w-2 rounded-full', toneAccent[tone])} aria-hidden />
      </div>
      <div className={cn('relative min-h-[148px] bg-gradient-to-br p-3', toneSurface[tone])}>
        {children}
      </div>
    </div>
  )
...
function PreviewBody({ variant }: { variant: ProductPreviewVariant }) {
  switch (variant) {
        ...
        <div className="grid grid-cols-[56px_1fr] gap-2">
          <div className="rounded-[6px] bg-background/80 p-1.5 shadow-sm">
            ...
            <div className="space-y-1.5">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-1.5 rounded-full bg-[rgba(55,53,47,0.12)] dark:bg-[rgba(255,255,255,0.12)]" />
              ))}
            </div>
          </div>
    ...
    case 'forms':
      return (
        <div className="rounded-[8px] bg-background/90 p-3 shadow-sm">
          ...
          <div className="space-y-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="rounded-[6px] border border-[rgba(55,53,47,0.08)] bg-[rgba(55,53,47,0.03)] px-2 py-2 dark:border-[rgba(255,255,255,0.08)]">
        ...
        <div className="grid grid-cols-[72px_1fr] gap-2">
          <div className="space-y-1.5 rounded-[6px] bg-background/85 p-2 shadow-sm">
            {['GET', 'POST', 'PUT'].map((method) => (
              <div key={method} className="rounded-[4px] bg-[rgba(55,53,47,0.06)] px-1.5 py-1 text-[9px] font-semibold tracking-wide text-foreground/70 dark:bg-[rgba(255,255,255,0.06)]">
                {method}
              </div>
            ))}
          </div>
          <div className="rounded-[6px] bg-[#1e1e1e] p-2.5 font-mono text-[9px] leading-relaxed text-[#d4d4d4] shadow-sm">
            ...
            <div className="pl-2">method: <span className="text-[#ce9178]">'GET'</span>,</div>
            <div className="pl-2">path: <span className="text-[#ce9178]">'/users'</span></div>
            <div>{'}'})</div>
          </div>
        </div>
      )
    case 'licenses':
      return (
        <div className="rounded-[8px] bg-background/90 p-3 shadow-sm">
          ...
          <div className="space-y-1.5">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex items-center gap-2 rounded-[5px] bg-[rgba(55,53,47,0.04)] px-2 py-1.5 dark:bg-[rgba(255,255,255,0.04)]">
    ...
    case 'canvas':
      return (
        <div className="relative h-[132px] overflow-hidden rounded-[8px] bg-background/80 shadow-sm">
          ...
          <div className="absolute top-4 left-4 h-14 w-20 rounded-[8px] border border-[rgba(105,64,165,0.35)] bg-[rgba(105,64,165,0.18)] shadow-sm" />
          <div className="absolute top-10 right-6 h-16 w-24 rounded-[8px] border border-[rgba(35,131,226,0.35)] bg-[rgba(35,131,226,0.16)] shadow-sm" />
          <div className="absolute bottom-4 left-10 h-10 w-28 rounded-[8px] border border-[rgba(15,123,108,0.35)] bg-[rgba(15,123,108,0.16)] shadow-sm" />
        </div>
      )
    case 'library':
      return (
